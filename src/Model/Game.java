...
public class Game implements DemisableObserver{
	...
	public Game(Window window){
		...
		objects.add(new Player(10,10,3,5)); //(int x, int y, int maxBomb, int lifes)
		
		// Map building 
		for(int i = 0; i < size; i++){
			objects.add(new BlockUnbreakable(i,0));//en haut
			objects.add(new BlockUnbreakable(0,i));//à gauche
			objects.add(new BlockUnbreakable(i, size-1));//en bas
			objects.add(new BlockUnbreakable(size-1, i));// à droite
		}
